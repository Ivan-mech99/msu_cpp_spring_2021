...
void Empty_string_test() {
  cout << "Empty string test works" << endl;
  auto text = format("");
  assert(text == "");
  cout << "Done!" << endl;
}
...
void String_without_formatting_test() {
  cout << "String without formatting test" << endl;
...
void Very_big_numbers_test() {
  cout << "Very big numbers test" << endl;
  string text;
  try {
    text = format("{0}+{0} = {18446744073709551615}", 100, 243);
  } catch (too_big &e) {
    cout << e.what() << endl;
    text = e.what();
  }
  try {
    text = format("{0}+{0} = {28446744073709551615}", 100, 243);
  } catch (too_big &e) {
    cout << e.what() << endl;
    text = e.what();
  }
...
void Only_one_arg_string_test() {
  cout << "Only one arg string test" << endl;
  string text;
  text = format("{0}", 100);
  assert(text == "100");
  text = format("{0}", "just one arg\n");
  assert(text == "just one arg\n");
  cout << "Done!" << endl;
}
...
int main() {
  ...
  Extra_braces_test();
  Very_big_numbers_test();
  Only_one_arg_string_test();
  cout << "Success!" << endl;
}
