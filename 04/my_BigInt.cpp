...
int BigInt::find_len_help(int &num, int &l) {
  num = (num - (num % 1000)) / 1000;
  l++;
  return num;
}

int BigInt::collect_remainders(int &num) {
  int res = num % 1000;
  num = (num - (num % 1000)) / 1000;
  return res;
}
...
BigInt::BigInt(string number) {
  ...
  // cout << number << endl;
  // cout << inner_string << endl;
  if (inner_string.length() % 3 != 0) {
    num_len++;
  }
  // cout << "Show num_len: " << num_len << endl;
  num_data = new int[num_len];
  int j = 0;
  int k = -1;
  for (i = inner_string.length() - 1; i >= 0; i--) {
    // cout << "local: " <<i<<' '<< inner_string[i] << endl;
    temp = 0;
    if (j % 3 == 0) {
      // cout << "demo: " << inner_string[i] << endl;
      k = k + 1;
...
BigInt BigInt::summar(const BigInt &a, const BigInt &b) {
  int final_len;
  int min_len;
  int max_len;
  int i;
...
BigInt BigInt::differ(const BigInt &a, const BigInt &b) {
  ...
  for (i = 0; i < min_len; i++) {
    temp_data[i] = a.num_data[i] - b.num_data[i];
    if (temp_data[i] < 0) {
